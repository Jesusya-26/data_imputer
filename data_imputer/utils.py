...
def calculate_statistics(data: GeoDataFrame, statistics: str) -> GeoDataFrame:
    ...
    data = data.drop(["geometry"], axis=1)
    imputed_initial_vals = data.apply(
        lambda c: c.astype("object").fillna(
            getattr(c, statistics)() if getattr(c, statistics)() is not np.nan else 0
        )
    )
...
def set_initial_dtypes(
    ...
    if isinstance(data_to_round, Series):
        ...
        rounded_data = (
            data_to_round.round().astype(dt_neigh[column_name].name)
            if column_name in cols and "Int" in dt_neigh[column_name].name
            else data_to_round
        )
    else:
        rounded_data = data_to_round.apply(
            lambda c: (
                c.round() if c.name in cols and "Int" in dt_neigh[c.name].name else c
            )
        )
        rounded_data = rounded_data.apply(
            lambda c: c.astype(dt_neigh[c.name].name) if c.name in cols else c
        )
    return rounded_data
...
def sort_columns(
    data: DataFrame | GeoDataFrame, sort_type: str
...
def define_nans_positions(data: DataFrame | GeoDataFrame) -> dict:
